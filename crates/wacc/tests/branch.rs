...
#[test]
fn test_branch_wast() {
    let _s = span!(Level::INFO, "test_branch_wast").entered();
    let kvp = Kvp::default();
    let mut pstack = Stk::default();
    let mut rstack = Stk::default();
    let script = include_str!(concat!(env!("OUT_DIR"), "/branch.wast"))
        .as_bytes()
        .to_vec();
...
#[test]
fn test_branch_wasm() {
    let _s = span!(Level::INFO, "test_branch_wasm").entered();
    let kvp = Kvp::default();
    let mut pstack = Stk::default();
    let mut rstack = Stk::default();
    let script = include_bytes!(concat!(env!("OUT_DIR"), "/branch.wasm")).to_vec();
    let mut instance = test_example(script, true, &kvp, &kvp, &mut pstack, &mut rstack);
    assert_eq!(b"/forks/child/pubkey\n".to_vec(), instance.log());
    let mut ctx = instance.store.as_context_mut();
    let context = ctx.data_mut();
