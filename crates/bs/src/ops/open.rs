        ...
        + ToString,
{
    ...
    let lock_script = {
        let lock_path = config
            .entry_lock_script
            .ok_or::<E>(OpenError::NoEntryLockScript.into())?;
    ...
    let unlock_script = {
        let unlock_path = config
            .entry_unlock_script
            .ok_or::<E>(OpenError::NoEntryUnlockScript.into())?;
    ...
    let log = provenance_log::log::Builder::new()
        .with_vlad(&vlad)
        .with_first_lock(&first_lock_script.ok_or::<E>(OpenError::NoFirstLockScript.into())?)
        .append_entry(&entry)
        .try_build()?;
