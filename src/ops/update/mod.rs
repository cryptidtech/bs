...
use provenance_log::{entry::{self, Entry}, error::EntryError, Error as PlogError, Key, Log, OpId};
use std::{fs::read, path::Path};

/// update a provenance log given the update config
pub fn update_plog<F1, F2>(plog: &mut Log, config: Config, get_key: F1, sign_entry: F2) -> Result<Entry, Error>
where
    ...
    F2: Fn(&Multikey, &[u8]) -> Result<Multisig, Error>,
{
    ...
    config.entry_ops.iter().try_for_each(|params| -> Result<(), Error> {
        match params {
    ...
    let unlock_script = {
        let unlock_path = config.entry_unlock_script.ok_or::<Error>(UpdateError::NoEntryUnlockScript.into())?;
        script::Loader::new(&unlock_path).try_build()?
    };

    // get the entry signing key
    let entry_mk = config.entry_signing_key.ok_or::<Error>(UpdateError::NoSigningKey.into())?;
